<template>
  <div class="traffic">
    ...
    <template v-else>
      <p class="meta">
        口径：{{ board.label }} · 门店 {{ board.stores.length }} 家
        <span v-if="board.prevFunnel"> · 环比对照上一同等跨度</span>
      </p>

      <section class="card">
        <div class="sec-head"><span class="no">1</span>流量漏斗</div>
        <div class="funnel">
      ...
      <div class="grid-2">
        ...
        <section class="card">
          <div class="sec-head"><span class="no">3</span>活动贡献</div>
          <div class="empty-box">
            <strong>本期无活动数据源</strong>
            <p>9月日更目录未包含「活动·店铺活动」文件，不展示编造的 ROI / 补贴 / 新老客。</p>
          </div>
        </section>
      </div>

      <section class="card">
        <div class="sec-head"><span class="no">4</span>门店流量效率（按整体转化升序）</div>
        <div class="scroll">
          <table>
            <thead>
              <tr>
...
function rateTone(v: number | null, kind: 'enter' | 'order') {
  if (v == null) return ''
  const line = kind === 'enter' ? 0.08 : 0.2
  return v < line ? 'bad' : ''
}
</script>
