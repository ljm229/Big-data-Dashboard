...
function percent(v:number|null|undefined){return v==null?'—':`${(v*100).toFixed(2)}%`}
...
const deliveryOpt=computed<any>(()=>{const rows=board.value?.deliveryDaily||[];const pct=(row:any,key:string)=>{const t=row.timely+row.late+row.missing;return t?row[key]/t*100:0};return{grid:{left:46,right:22,top:36,bottom:34},legend:{top:3,right:5},tooltip:{trigger:'axis',formatter:(ps:any[])=>`${ps[0]?.axisValue}<br/>${ps.map(p=>`${p.marker}${p.seriesName} ${p.value.toFixed(2)}%`).join('<br/>')}`},xAxis:{type:'category',data:rows.map(x=>x.day.slice(5)),axisTick:{show:false}},yAxis:{type:'value',max:100,axisLabel:{formatter:'{value}%'},splitLine:{lineStyle:{color:'#edf2f7'}}},series:[{name:'及时',type:'bar',stack:'all',data:rows.map(x=>pct(x,'timely')),itemStyle:{color:'#14B8A6'}},{name:'不及时',type:'bar',stack:'all',data:rows.map(x=>pct(x,'late')),itemStyle:{color:'#EF5B5B'}},{name:'缺少节点',type:'bar',stack:'all',data:rows.map(x=>pct(x,'missing')),itemStyle:{color:'#F59E0B',borderRadius:[3,3,0,0]}}]}})
const storeOpt=computed<any>(()=>{const rows=board.value?.stores||[];return{grid:{left:52,right:24,top:28,bottom:46},tooltip:{formatter:(p:any)=>`${p.name}<br/>逆向 ${formatInt(p.value[0])} 单<br/>配送不及时率 ${p.value[1].toFixed(2)}%<br/>逆向金额 ${money(p.value[2])}`},xAxis:{name:'逆向订单',splitLine:{lineStyle:{color:'#edf2f7'}}},yAxis:{name:'不及时率',axisLabel:{formatter:'{value}%'},splitLine:{lineStyle:{color:'#edf2f7'}}},series:[{type:'scatter',data:rows.map(x=>{const r=x.deliveryTotal?x.late/x.deliveryTotal*100:0;return{name:x.shortName,value:[x.orderCnt,r,x.amount],itemStyle:{color:r>=30?'#EF5B5B':r>=20?'#F59E0B':'#14B8A6'}}}),symbolSize:(v:number[])=>Math.max(10,Math.min(34,Math.sqrt(v[0]||0)*1.2))}]}})
