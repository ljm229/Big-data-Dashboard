...
<template>
          ...
          <TopBar data-role="nav" data-height-px="64" data-single-line="true">
            ...
            <template #nav>
              <div class="view-switch view-switch--in-header">
                ...
                <button type="button" @click="activeView = 'upload'">数据上传</button>
              </div>
            </template>
          </TopBar>
        </section>
  ...
  <DataUploadPage v-else-if="activeView === 'upload'" />
  <StoreBoard
...
import DataUploadPage from './components/classic-pages/DataUploadPage.vue'
import DateFilter from './components/DateFilter.vue'

const boot = new URLSearchParams(typeof location === 'undefined' ? '' : location.search)
const activeView = ref<'cockpit' | 'ops' | 'upload'>(boot.get('view') === 'ops' ? 'ops' : boot.get('view') === 'upload' ? 'upload' : 'cockpit')
