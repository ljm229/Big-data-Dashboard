...
export function useChart(elRef: Ref<HTMLElement | null>, option: Ref<any>) {
  ...
  function ensure() {
    ...
    // v-if 切换页签会销毁重建 DOM，旧实例仍绑在已卸载的节点上，需销毁重建
    if (chart.value && boundEl !== el) {
      chart.value.dispose()
      chart.value = null
      boundEl = null
    }
  ...
  watch(option, async (opt) => {
    await nextTick()
    if (!opt) return
    // 页签 v-if 重建 DOM 后实例仍绑在已卸载节点上，走 ensure 重建
    if (!chart.value || boundEl !== elRef.value) {
      ensure()
      return
    }
    applyOption(opt)
