...
export type Source1Store = { name: string; city: string; status: string; address?: string; isNew?: boolean | null }
type Fact = {
  date: string
  channel: string
  store: string
...
const isValidStore = (store: Source1Store) => {
  const name = String(store.name || '').trim()
  return Boolean(name) && !/^(门店|门店名称)$/.test(name) && !/模板店/.test(name) && !/优沃森超市/.test(name)
}
...
export const STORE_AGE_COLOR = { new: '#22d3ee', growing: '#f59e0b', old: '#818cf8' } as const
export type StoreAge = 'new' | 'growing' | 'old' | 'unknown'

const storeFirstDay = new Map<string, string>()
for (const r of facts) {
  const k = normStoreName(r.store)
  const cur = storeFirstDay.get(k)
  if (!cur || r.date < cur) storeFirstDay.set(k, r.date)
}
function dayDiff(a: string, b: string) {
  return Math.round((Date.parse(a) - Date.parse(b)) / 86400000)
}
/** 新老划分：开业≤90天新店，91-180爬坡，>180老店；无首日返回unknown */
export function storeAgeOf(store: string, at: string): StoreAge {
  const first = storeFirstDay.get(normStoreName(store))
  if (!first || !at) return 'unknown'
  const d = dayDiff(at, first)
  if (d < 0) return 'unknown'
  if (d <= 90) return 'new'
  if (d <= 180) return 'growing'
  return 'old'
}
export function storeAgeLabel(a: StoreAge) {
  return a === 'new' ? '新店' : a === 'growing' ? '爬坡店' : a === 'old' ? '老店' : '未知'
}
/** 是否新店：取上线进度表「是否新店」列；null=表里没标 */
export function storeIsNewFlag(store: string): boolean | null {
  const hit = lookupStore(store)
  if (!hit) return null
  return hit?.isNew ?? null
}
/** 新老身份：上线进度表「是否新店」=否就是老店，其余（是/空）都算新店 */
export type StoreCohort = 'new' | 'old'
export function storeCohortOf(_store: string): StoreCohort {
  return storeIsNewFlag(_store) === false ? 'old' : 'new'
}
export function storeCohortLabel(c: StoreCohort) {
  return c === 'new' ? '新店' : c === 'old' ? '老店' : '未标注'
}
/** 门店×渠道聚合，供新店诊断使用 */
export function source1ByStoreChannel(filter: KpiFilter) {
  return fold(scopedFacts(filter), (r) => `${normStoreName(r.store)}\t${r.channel}`).map((r) => {
    const [storeKey, channelName] = r.key.split('\t')
    const hit = lookupStore(storeKey)
    return { ...r, store: hit?.name || storeKey, channel: channelName }
  })
}
/** 城市×渠道聚合，供桑基/哑铃使用 */
export function source1ByCityChannel(filter: KpiFilter) {
  return fold(scopedFacts(filter), (r) => `${canonCity(lookupStore(r.store)?.city || '')}\t${r.channel}`).map((r) => {
    const [city, channelName] = r.key.split('\t')
    return { ...r, city, channel: channelName }
  })
}
/** 新老聚合：毛利/订单/门店数 */
export function source1ByAge(filter: KpiFilter) {
  const at = filter.to
  const groups = new Map<StoreAge, Fact[]>()
  for (const r of scopedFacts(filter)) {
    const a = storeAgeOf(r.store, at)
    const list = groups.get(a) || []
    list.push(r)
    groups.set(a, list)
  }
  return (['new', 'growing', 'old', 'unknown'] as StoreAge[]).map((a) => {
    const rows = groups.get(a) || []
    const stores = new Set(rows.map((r) => normStoreName(r.store))).size
    return { age: a, label: storeAgeLabel(a), stores, ...sumRows(rows) }
  })
}
