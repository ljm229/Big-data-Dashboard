  ...
  <div class="ck-page">
      ...
      <article class="ck-card">
        <header class="ck-card__head">
          ...
          <table class="ck-table">
            <thead>
              <tr><th>考核指标</th><th>实际值</th><th>环比</th><th>达标阈值</th><th>状态</th></tr>
            </thead>
            <tbody>
                <tr v-for="p in selectedRow.parts" :key="p.key" :class="{ focus: focusMetric === p.key }">
                  ...
                  <td><span class="wow" :class="wowTone(p)">{{ wowText(p) }}</span></td>
    ...
    <section class="ck-grid-quality-bot">
      <article class="ck-card dist-card">
        <header class="ck-card__head">
          <h3>门店上升下降情况</h3>
          <p>综合分较{{ compareHintPrefix }}</p>
        </header>
        <div class="dist-scroll">
          <div ref="distEl" class="dist-chart" :style="{ height: distH + 'px' }" />
        </div>
              </article>
...
/** 门店上升下降：本期综合分较上期的变化（真实考核行） */
const storeMoves = computed(() => {
  const prev = prevBoard.value
  if (!prev) return [] as Array<{ key: string; label: string; before: number; after: number; delta: number }>
  const prevMap = new Map(
    prev.rows.filter((r: any) => !isMissingAssessRow(r)).map((r: any) => [r.shortName, r.composite as number]),
  )
  return assessRows.value
    .filter((r) => !isMissingAssessRow(r) && prevMap.has(r.shortName))
    .map((r) => {
      const before = prevMap.get(r.shortName)!
      return { key: r.shortName, label: short(r.shortName), before, after: r.composite, delta: r.composite - before }
    })
    .sort((a, b) => b.delta - a.delta)
})

const distRows = computed(() => storeMoves.value.slice(0, 20))
const distH = computed(() => Math.max(220, distRows.value.length * 30 + 70))

function buildDistChart() {
  const moves = distRows.value
  if (!moves.length) {
    distOpt.value = {
      title: {
        text: '暂无上期对比数据',
  ...
  const rows = moves.slice().reverse()
  distOpt.value = {
    ...
    tooltip: {
      ...
      formatter: (params: any[]) => {
        ...
        return `<b>${m.label}</b><br/>本期 ${m.after.toFixed(1)} · 上期 ${m.before.toFixed(1)}<br/>变化 <b>${sign}${m.delta.toFixed(1)}</b>`
      },
    },
    ...
    series: [
      {
        ...
        data: rows.map((m) => ({
          key: m.key,
          value: Number(m.delta.toFixed(2)),
          itemStyle: {
            color: m.delta > 0.5 ? '#22c55e' : m.delta < -0.5 ? '#ef4444' : '#cbd5e1',
            borderRadius: m.delta > 0 ? [0, 6, 6, 0] : [6, 0, 0, 6],
          },
        })),
        label: {
          show: true,
          ...
          fontSize: 11,
          fontWeight: 700,
          formatter: (q: { value: number }) => `${q.value > 0 ? '+' : ''}${q.value.toFixed(1)}`,
        },
      },
    ],
  }
...
/** 门店详情环比：本期实际值 vs 上期同门店同指标。越低越好指标反向着色。无上期显示 —。 */
const prevPartsMap = computed(() => {
  const m = new Map<string, number>()
  const row = prevBoard.value?.rows?.find((r: any) => r.shortName === selectedStoreId.value)
  for (const p of row?.parts || []) {
    if (p && !p.missing && Number.isFinite(p.value)) m.set(p.key, p.value)
  }
  return m
})
function wowText(p: { key: AssessKey; value: number; missing?: boolean; lowerBetter?: boolean; unit?: string }) {
  if (p.missing) return '—'
  const prev = prevPartsMap.value.get(p.key)
  if (prev == null || !Number.isFinite(prev)) return '—'
  const diff = p.value - prev
  if (Math.abs(diff) < 1e-9) return '持平'
  const sign = diff > 0 ? '+' : ''
  if (p.unit === 'min') return `${sign}${diff.toFixed(1)}`
  return `${sign}${diff.toFixed(2)}pp`
}
function wowTone(p: { key: AssessKey; value: number; missing?: boolean; lowerBetter?: boolean }) {
  if (p.missing) return ''
  const prev = prevPartsMap.value.get(p.key)
  if (prev == null) return ''
  const diff = p.value - prev
...
function buildTrend() {
  ...
  trendOpt.value = {
    ...
    tooltip: {
      ...
      borderWidth: 0,
      padding: [8, 12],
      textStyle: { color: '#111113', fontSize: 12 },
      extraCssText: 'box-shadow:0 4px 24px rgba(0,0,0,.08);border-radius:12px;',
      formatter: (params: any[]) => {
    ...
    series: [
      {
        ...
        symbolSize: 6,
        data: pointData,
        lineStyle: { width: 2.5, color: '#2563FF' },
        itemStyle: { color: '#2563FF', borderWidth: 2, borderColor: '#fff' },
        areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(37,99,255,.18)' }, { offset: 1, color: 'rgba(37,99,255,0)' }] } },
...
watch([storeMoves], () => buildDistChart(), { deep: true })
</script>
...
.dist-scroll { max-height: 430px; overflow-y: auto; }
.dist-all-btn {
  flex: none; display: flex; align-items: baseline; gap: 6px;
  border: 1px solid #bbe7c9; background: #f0faf3; color: #14532d;
...
.cell {
  ...
  border-radius: 999px;
  padding: 0;
  font: 600 11px/1 var(--ck-font-num);
  cursor: pointer;
  background: #F1F1F3;
  color: #6E6E73;
  &.ok { background: rgba(0,195,137,.12); color: #00C389; }
  &.warn { background: rgba(255,138,0,.14); color: #FF8A00; }
  &.bad { background: rgba(255,77,109,.12); color: #FF4D6D; }
  &.miss { background: #F1F1F3; color: #AEAEB2; }
}
...
.wow { font-size: 11px; font-weight: 700; font-variant-numeric: tabular-nums; color: #94a3b8; white-space: nowrap; }
.wow.up { color: #16a34a; }
.wow.down { color: #dc2626; }
.wow.flat { color: #64748b; }
...
.advice-list {
  ...
  /* 仅蓝 / 橙 / 红三态，避免多余色相 */
  .tone-0 { strong { background: rgba(37,99,255,.10); color: #2563FF; } }
  .tone-1 { strong { background: rgba(255,138,0,.14); color: #FF8A00; } }
  .tone-2 { strong { background: rgba(255,77,109,.12); color: #FF4D6D; } }
  .tone-3 { strong { background: rgba(0,195,137,.12); color: #00C389; } }
  .tone-4 { strong { background: rgba(255,77,109,.12); color: #FF4D6D; } }
}
