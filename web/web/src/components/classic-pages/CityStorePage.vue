...
<template>
  <div class="ck-page city-page">
    <section v-if="cityStoreDimension === 'ops'" class="ck-kpi-row">
      <ClassicKpi
    ...
    <template v-if="cityStoreDimension === 'ops'">
      <section class="ck-grid-city-bot">
      <article class="ck-card">
        <header class="ck-card__head">
            <h3>渠道门店分析 · 城市 → 渠道</h3>
            <p>原渠道门店页已并入本页 · 线宽=贡献毛利 · 点击渠道筛选</p>
        </header>
        <div v-show="sankeyLinks.length" ref="sankeyEl" class="ck-plot ck-plot--chart ck-plot--tall" />
        <div v-if="!sankeyLinks.length" class="ck-empty"><b>暂无渠道数据</b></div>
        <div v-if="channelRows.length" class="ch-chips">
          <button v-for="r in channelRows" :key="r.key" type="button" class="ch-chip" :class="{ selected: channel === r.key }" @click="setChannel(r.key)">
            <i :style="{ background: chColor(r.key) }" />{{ r.key }}<b>{{ fmtWan(r.profit) }}</b><small>{{ fmtPct(r.profitRate) }}</small>
          </button>
          <button v-if="channel !== '全部'" type="button" class="ch-chip is-clear" @click="setChannel('全部')">清除</button>
        </div>
      </article>
      <article class="ck-card">
        <header class="ck-card__head"><h3>渠道偏科哑铃</h3><p>空心=订单占比 · 实心=毛利占比</p></header>
        <div v-show="dumbbellRows.length" ref="dumbbellEl" class="ck-plot ck-plot--chart ck-plot--tall" />
        <div v-if="!dumbbellRows.length" class="ck-empty"><b>暂无偏科数据</b></div>
      </article>
      <article class="ck-card">
        <header class="ck-card__head"><h3>新店 vs 老店</h3><p>≤90天新店 · 91-180爬坡 · &gt;180老店</p></header>
        <div v-if="ageRows.some((a) => a.stores)" class="age-list">
          <div v-for="a in ageRows" :key="a.age" class="ck-hbar">
            <span><i class="age-dot" :style="{ background: ageColor(a.age) }" />{{ a.label }} · {{ a.stores }}店</span>
            <i><em :style="{ width: a.barW, background: ageColor(a.age) }" /></i>
            <b>{{ fmtWan(a.profit) }}</b>
            <span class="pct">{{ fmtPct(a.profitRate) }} · {{ formatInt(a.orders) }}单</span>
          </div>
          <p class="age-note">{{ ageInsight }}</p>
        </div>
        <div v-else class="ck-empty"><b>暂无新老划分</b><span>缺首日数据标—</span></div>
      </article>
      </section>
      ...
      <article class="ck-card">
        <header class="ck-card__head">
            <h3>城市经营地图</h3>
          <div class="ck-pills">
              <button type="button" class="active" disabled>经营分布</button>
    ...
    <CoreMetricsTrackTab v-else />

    <footer v-if="cityStoreDimension === 'ops'" class="ck-foot">
      <span>门店营业概况</span>
...
import CoreMetricsTrackTab from './CoreMetricsTrackTab.vue'
...
import {
  ...
  source1ByChannel,
  source1ByCity,
  source1ByCityChannel,
  source1ByStore,
  source1LaunchByCity,
  source1StoreCity,
  storeAgeLabel,
} from '../../api/source1'
import { cityStoreDimension } from '../../api/cityStoreView'
...
const dumbbellEl = ref<HTMLElement | null>(null)
const dumbbellOption = ref<any>(null)
const { chart: sankeyChart } = useChart(sankeyEl, sankeyOption)
const { chart: dumbbellChart } = useChart(dumbbellEl, dumbbellOption)
function chColor(name: string) {
  return CHANNEL_COLOR[name] || '#94a3b8'
}
function ageColor(age: string) {
  return (STORE_AGE_COLOR as Record<string, string>)[age] || '#94a3b8'
}
function setChannel(name: string) {
  if (channel.value === name) filter.setChannel('全部')
  else filter.setChannel(name)
}
const channelRows = computed(() => source1ByChannel(nationalQ.value).filter((r) => r.profit != null || r.orders != null).sort((a, b) => (b.profit || 0) - (a.profit || 0)))
const cityChannelRows = computed(() => source1ByCityChannel(nationalQ.value).filter((r) => r.city && r.channel && r.profit != null).slice(0, 24))
const sankeyLinks = computed(() => cityChannelRows.value.map((r) => ({ source: shortCity(r.city), target: r.channel, value: Math.max(Math.abs(r.profit || 0), 1), profit: r.profit })))
const dumbbellRows = computed(() => {
  const totalProfit = channelRows.value.reduce((a, r) => a + Math.abs(r.profit || 0), 0) || 1
  const totalOrders = channelRows.value.reduce((a, r) => a + Math.abs(r.orders || 0), 0) || 1
  return channelRows.value.map((r) => ({
    name: r.key,
    orderShare: Math.abs(r.orders || 0) / totalOrders,
    profitShare: Math.abs(r.profit || 0) / totalProfit,
    color: chColor(r.key),
  }))
})
const ageRows = computed(() => {
  const rows = source1ByAge(q.value)
  const max = Math.max(...rows.map((r) => Math.abs(r.profit || 0)), 1)
  return rows.map((r) => ({ ...r, barW: `${Math.max(6, Math.round((Math.abs(r.profit || 0) / max) * 100))}%` }))
})
const ageInsight = computed(() => {
  const get = (a: string) => ageRows.value.find((r) => r.age === a)
  const n = get('new')
  const o = get('old')
  if (!n?.stores) return '期内无新店毛利贡献，老店承担全部利润。'
  if (n.profit != null && n.profit < 0) return `新店${n.stores}家合计亏损${fmtWan(n.profit)}，处于投入期，重点盯爬坡转化。`
  if (o?.profit && n.profit != null && n.profit > o.profit) return `新店毛利已超老店，新店成增长主力。`
  return `新店${n.stores}家贡献${fmtWan(n.profit)}，老店${o?.stores || 0}家基本盘稳定。`
})
watch([sankeyLinks], () => {
  if (!sankeyLinks.value.length) { sankeyOption.value = null; return }
  const cities = [...new Set(cityChannelRows.value.map((r) => shortCity(r.city)))]
  const channels = [...new Set(cityChannelRows.value.map((r) => r.channel))]
  sankeyOption.value = {
    tooltip: { trigger: 'item', formatter: (p: any) => p.dataType === 'edge' ? `${p.data.source} → ${p.data.target}<br/>毛利 ${fmtWan(p.data.profit)}` : p.name },
    series: [{
      type: 'sankey', left: 8, right: 8, top: 10, bottom: 10, nodeWidth: 12, nodeGap: 10,
      label: { color: '#334155', fontSize: 11, fontWeight: 600 },
      lineStyle: { color: 'gradient', opacity: 0.55, curveness: 0.5 },
      data: [...cities.map((name) => ({ name, itemStyle: { color: '#64748b' } })), ...channels.map((name) => ({ name, itemStyle: { color: chColor(name) } }))],
      links: sankeyLinks.value.map((l) => ({ ...l, lineStyle: { color: chColor(l.target) } })),
    }],
  }
}, { immediate: true })
watch([dumbbellRows], () => {
  if (!dumbbellRows.value.length) { dumbbellOption.value = null; return }
  dumbbellOption.value = {
    grid: { left: 76, right: 40, top: 12, bottom: 28 },
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' }, formatter: (ps: any[]) => ps.map((p) => `${p.marker}${p.seriesName} ${(Number(p.value) * 100).toFixed(1)}%`).join('<br/>') },
    xAxis: { type: 'value', max: 1, axisLabel: { formatter: (v: number) => `${(v * 100).toFixed(0)}%`, color: '#64748b' }, splitLine: { lineStyle: { color: '#eef2f7' } } },
    yAxis: { type: 'category', data: dumbbellRows.value.map((r) => r.name), axisLabel: { color: '#334155', fontWeight: 600 } },
    series: [
      { name: '订单占比', type: 'scatter', symbol: 'circle', symbolSize: 12, itemStyle: { color: '#fff', borderColor: '#334155', borderWidth: 2 }, data: dumbbellRows.value.map((r, i) => [r.orderShare, i]) },
      { name: '毛利占比', type: 'scatter', symbol: 'circle', symbolSize: 14, itemStyle: { color: dumbbellRows.value[0]?.color || '#ff6a00', borderColor: '#fff', borderWidth: 2, shadowBlur: 6, shadowColor: 'rgba(0,0,0,.18)' }, data: dumbbellRows.value.map((r, i) => ({ value: [r.profitShare, i], itemStyle: { color: r.color } })) },
    ],
  }
  void sankeyChart; void dumbbellChart
}, { immediate: true })
...
.ck-plot--tall { height: 240px; min-height: 240px; }
.ch-chips { display: flex; flex-wrap: wrap; gap: 6px; padding: 8px 12px 12px; }
.ch-chip {
  display: inline-flex; align-items: center; gap: 6px;
  height: 30px; padding: 0 10px; border: 1px solid var(--ck-line, #e5e7eb);
  border-radius: 999px; background: #fff; color: #334155; font-size: 12px; cursor: pointer;
  i { width: 8px; height: 8px; border-radius: 50%; }
  b { color: #1d4ed8; font-family: var(--ck-font-num); }
  small { color: #059669; }
  &.selected { border-color: #1d6bff; background: #eff6ff; }
  &.is-clear { color: #64748b; }
}
.age-list { display: flex; flex-direction: column; gap: 14px; justify-content: center; flex: 1; padding: 4px 2px; }
.age-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; margin-right: 6px; }
.age-note { margin: 2px 0 0; color: #475569; font-size: 12px; line-height: 1.5; border-left: 3px solid #1d6bff; background: #f2f7ff; padding: 8px 10px; border-radius: 0 8px 8px 0; }
</style>
