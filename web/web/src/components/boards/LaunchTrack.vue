...
<template>
  <div class="launch-shell">
    <Panel title="门店上线进度跟踪" :empty="!summary.plan">
      <template #extra><span class="launch-note">上线台账 · 独立展示 · 非日切</span></template>
      <div class="launch">
        <div class="kpis">
    ...
    <Teleport to="body">
      <div v-if="dialog" class="mask" @click.self="dialog = null">
        <section class="dialog" role="dialog" aria-modal="true">
          ...
          <p class="hint">共 {{ dialogRows.length }} 家</p>
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                <tr v-for="row in dialogRows" :key="row.name">
                  <td>{{ row.name }}</td>
                  <td>{{ row.city || '--' }}</td>
                  <td>{{ row.status }}</td>
...
/** 上线台账独立展示：不受其他区域城市/门店联动影响，始终展示全部门店 */
const rows = computed(() =>
...
const dialogRows = computed(() => {
  ...
  const metrics = source1ByStore({
    from: filter.periodRange.from,
    to: filter.periodRange.to,
    city: '全国',
    channel: filter.channel,
  })
  const map = new Map(metrics.map((m) => [m.key, m]))
  return source1StoresByStatus(dialog.value, '全国', '全部').map((s) => ({
    ...s,
...
.dialog table tr { cursor: default; }
.dialog table tr:hover { background: transparent; }
</style>
