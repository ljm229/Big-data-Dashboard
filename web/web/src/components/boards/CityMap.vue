...
/* 门店卡与城市卡一致：悬浮压在地图上，不压缩地图，保证透明底能透出底图 */
...
.detail-card {
  ...
  /* 与图2城市卡一致的透明诊断卡：深蓝渐变 + 亮蓝边框，不再用实心 panel 盖住底图 */
  background: linear-gradient(180deg, rgba(6, 38, 74, 0.88), rgba(3, 22, 46, 0.86));
  border: 1px solid rgba(94, 180, 255, 0.65);
  border-left: 3px solid var(--primary-2);
  border-radius: 4px;
  box-shadow: 0 0 14px rgba(0, 86, 138, 0.55), inset 0 1px 0 rgba(133, 239, 255, 0.35);
  pointer-events: auto;
  scrollbar-width: thin;
  scrollbar-color: #147aae rgba(3, 22, 46, 0.4);
  ...
  footer { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; margin-top: 8px; position: sticky; bottom: 0; padding-top: 6px; background: linear-gradient(180deg, transparent, rgba(3, 22, 46, 0.9) 30%); }
}
.detail-card--right {
  left: auto;
  right: 8px;
