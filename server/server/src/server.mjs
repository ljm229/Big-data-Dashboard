...
function splitLocFilter(value = '') {
  const raw = String(value || '').trim()
  if (!raw || raw === '全部' || raw === '全国' || raw === 'all') return []
  return [...new Set(raw.split(/[|、,，]/).map((x) => x.trim()).filter((x) => x && x !== '全部' && x !== '全国' && x !== 'all'))]
}
...
async function qualityRows(periodKey, city = '全部', store = '全部') {
  ...
  const cities = splitLocFilter(city)
  if (cities.length) {
    const parts = cities.map((item) => {
      params.push(item)
      const i = params.length
      return `(s.city = $${i} OR replace(s.city,'市','') = replace($${i},'市',''))`
    })
    clauses.push(`(${parts.join(' OR ')})`)
  }
  const stores = splitLocFilter(store)
  if (stores.length) {
    const parts = stores.map((item) => {
      params.push(item)
      const i = params.length
      return `(q.store_key = $${i} OR s.store_name = $${i} OR s.short_name = $${i})`
    })
    clauses.push(`(${parts.join(' OR ')})`)
  }
...
function buildReport(periodKey, currentRows, previousRows) {
  ...
  const ui = period.kind === 'day'
    ? { cur: '本日', prev: '昨日', delta: '日比' }
    : period.kind === 'month'
      ? { cur: '本月', prev: '上月', delta: '月比' }
      : { cur: '本周', prev: '上周', delta: '周比' }
